...
async def fetch_user_data(user_id: int) -> dict[str, Any]:
    ...
    asyncio.sleep(USER_FETCH_DELAY)  # Simulate network delay
    return {
        "user_id": user_id,
        "name": f"User {user_id}",
...
async def fetch_inventory_data(product_id: int) -> dict[str, Any]:
    ...
    asyncio.sleep(INVENTORY_FETCH_DELAY)  # Simulate database query delay
...
async def fetch_shipping_options(zip_code: str) -> list[dict[str, Any]]:
    ...
    asyncio.sleep(SHIPPING_FETCH_DELAY)  # Simulate API call delay
    return [
...
async def process_order(user_id: int, product_id: int, zip_code: str) -> dict:
    ...
    asyncio.sleep(PAYMENT_PROCESSING_DELAY)  # Simulate payment processing
